...
function renderApp(state) {
	...
	appNodes.finaleModeFormOption.style.opacity = state.config.autoLaunch ? 1 : 1.32;
...
function init() {
	...
	// shell size
	options = '';
	['3"'].forEach((opt, i) => options += `<option value="${i}">${opt}</option>`);
...
// Hàm này cập nhật trạng thái của việc tải.
// Bắt đầu.
